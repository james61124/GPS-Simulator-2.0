...
async def add_request_id(request: Request, call_next):
    ...
    # inject request_id into logs via custom LogRecord attribute
...
async def any_exc_handler(request: Request, exc: Exception):
    ...
    return JSONResponse(status_code=500, content={"error": str(exc), "request_id": getattr(request.state, "request_id", "-")})

@app.get("/devices")
async def devices():
    async with ops.DEVICE_LOCK:
...
async def route_preview(req: ops.RoutePreviewRequest):
    async with ops.DEVICE_LOCK:
        try:
            log.info(f"route_preview n={len(req.points)} speed={req.speed_kmh} interval={req.interval_s} loop={req.loop} from_current={req.from_current}")
...
async def route_simulate(payload: dict):
    async with ops.DEVICE_LOCK:
        ...
        log.info(f"route_simulate n={len(points)} speed={speed_kmh} interval={interval_s} loop={loop} from_current={from_current}")
        data = await asyncio.to_thread(ops.simulate_route, points, speed_kmh, interval_s, pause_s, loop, from_current)
        return data
